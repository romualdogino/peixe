...
export default function EditarTanquePage() {
  useRequireAuth()
  ...
  const fetchTanque = async () => {
    try {
      ...
      if (data.success) {
        const tanque = data.data
        setFormData({
          nome: tanque.nome || '',
          codigo_interno: tanque.codigo_interno || '',
          volume_m3: tanque.volume_m3?.toString() || '',
          area_m2: tanque.area_m2?.toString() || '',
          tipo: tanque.tipo || 'CIRCULAR',
          tipo_material: tanque.tipo_material || '',
          status: tanque.status || 'ATIVO',
          localizacao: tanque.localizacao || '',
          data_instalacao: tanque.data_instalacao ? new Date(tanque.data_instalacao).toISOString().split('T')[0] : '',
          nivel_agua_cm: tanque.nivel_agua_cm?.toString() || '',
          isActive: tanque.isActive !== undefined ? tanque.isActive : true
        })
      } else {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (!response.ok) {
        throw new Error(data.error || 'Erro ao atualizar tanque')
      }

      if (data.success) {
        router.push(`/tanques/${id}`)
      } else {
        throw new Error(data.error || 'Erro ao atualizar tanque')
      }
    } catch (error) {
    ...
    <div className="p-6 max-w-4xl mx-auto">
      ...
      <div className="mb-6">
        <div className="flex items-center space-x-2 text-sm text-gray-600 mb-2">
          ...
          <span>›</span>
          <Link href={`/tanques/${id}`} className="hover:text-primary-600 transition-colors">
            {formData.nome}
          </Link>
          <span>›</span>
        ...
        <div className="flex items-center justify-between">
          <div>
            <h1 className="text-3xl font-bold text-gray-900">Editar Tanque</h1>
            <p className="text-gray-600 mt-1">Atualize as informações do tanque</p>
          </div>
      ...
      {/* Form */}
      <form id="tanque-form" onSubmit={handleSubmit} className="space-y-6">
        {error && (
        ...
        <div className="bg-white rounded-lg border border-gray-200 p-6">
          ...
          <div className="mt-6">
            ...
            <p className="text-sm text-gray-500 mt-1">
              Desmarque para desativar o tanque temporariamente
            </p>
          </div>
        </div>
      </form>
